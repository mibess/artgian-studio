...
const groups = [
  ...
  },
  {
    ...
    items: [
      ...
      { href: "/admin/funil", label: "Funil", icon: Workflow },
      { href: "/admin/conversas", label: "Conversas", icon: MessageCircleMore },
  ...
  },
  {
    ...
    items: [
      ...
      { href: "/admin/campanhas", label: "Campanhas", icon: Tags },
      { href: "/admin/experimentos", label: "Experimentos", icon: FlaskConical },
    ],
  },
  {
    label: "Operação",
    items: [
      { href: "/admin/ia", label: "Inteligência artificial", icon: Bot },
      { href: "/admin/jobs", label: "Automações", icon: Gauge },
      { href: "/admin/excecoes", label: "Revisão humana", icon: CircleAlert },
  ...
  const sidebar = (
    <>
      <div className="flex h-[82px] items-center justify-between border-b border-white/10 px-6">
        <Link className="group flex items-center gap-3" href="/admin" onClick={() => setOpen(false)}>
          ...
          <span>
            ...
            <span className="block text-[10px] font-medium uppercase tracking-[0.18em] text-white/45">Painel administrativo</span>
          </span>
        </Link>
      ...
      <nav aria-label="Menu administrativo" className="commercial-scrollbar flex-1 overflow-y-auto px-3 py-5">
        {groups.map((group) => (
          <div className="mb-5" key={group.label}>
            ...
            <div className="space-y-1">
              {group.items.map((item) => {
                ...
                return (
                  <Link key={item.href} href={item.href} aria-current={active ? "page" : undefined} onClick={() => setOpen(false)} className={`group flex h-10 items-center gap-3 rounded-xl px-3 text-[13px] font-medium transition ${active ? "bg-[#ee6e4f] text-white shadow-[0_8px_20px_rgba(238,110,79,.18)]" : "text-white/58 hover:bg-white/7 hover:text-white"}`}>
      ...
      <div className="border-t border-white/10 p-4">
        ...
        <form action={signOutAdmin} className="mt-3">
          <SubmitButton pendingLabel="Saindo…" className="flex w-full items-center justify-center gap-2 rounded-xl px-4 py-2 text-xs font-medium text-white/60 hover:bg-white/7 hover:text-white disabled:opacity-60">
