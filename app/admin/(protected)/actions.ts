...
import { auditLogs, briefings, campaigns, commercialOrders, conversations, exceptions, experiments, jobs, leads, messages, outboundEvents, outboundProspects, quoteRequests, timelineEvents } from "../../../db/schema";
...
export async function simulateInbound(formData: FormData) {
  ...
  if (username.length < 2 || text.length < 2) {
    redirect("/admin/conversas?erro=Preencha+o+perfil+e+a+mensagem");
  }
  ...
  revalidatePath("/admin", "layout");
  redirect(`/admin/leads/${result.leadId}?simulado=1`);
}
...
export async function updateBusinessConfiguration(formData: FormData) {
  ...
  if (whatsappLink && whatsappLink !== "A_DEFINIR") {
    ...
    try {
      ...
      redirect("/admin/configuracoes?erro=Informe+um+link+completo+e+válido");
    }
    if (!['wa.me', 'api.whatsapp.com'].includes(url.hostname)) {
      redirect("/admin/configuracoes?erro=Use+um+link+oficial+do+WhatsApp");
    }
  }
...
function redirectCampaign(formData: FormData, query: string): never {
  ...
  redirect(`/admin/campanhas?${params.toString()}${anchor}`);
}
...
export async function createExperiment(formData: FormData) {
  ...
  if (
    ...
    redirect("/admin/experimentos?erro=Preencha+todos+os+campos+do+experimento");
  }
  const db = await getCommercialDb();
  const now = new Date().toISOString();
  await db.transaction(async (tx) => {
  ...
  redirect("/admin/experimentos?salvo=1");
}
...
export async function setExperimentStatus(formData: FormData) {
  ...
  if (!experimentId || !["active", "paused"].includes(status)) {
    redirect("/admin/experimentos?erro=Estado+de+experimento+inválido");
  }
  const db = await getCommercialDb();
  const [experiment] = await db
    .select()
    ...
    .limit(1);
  if (!experiment) redirect("/admin/experimentos?erro=Experimento+não+encontrado");
  const now = new Date().toISOString();
  await db.transaction(async (tx) => {
    if (status === "active") {
      await tx
  ...
  revalidatePath("/admin/experimentos");
  redirect("/admin/experimentos?estado=1");
}
...
export async function confirmCommercialOrder(formData: FormData) {
  ...
  revalidatePath(`/admin/leads/${leadId}`);
  revalidatePath("/admin/pedidos-comerciais");
...
export async function registerWhatsappHandoff(formData: FormData) {
  ...
  if (!handoff.enabled) redirect("/admin/configuracoes?erro=Configure+o+WhatsApp+antes+do+handoff");
  const now = new Date().toISOString();
  await db.transaction(async (tx) => {
...
export async function createInstagramReplyDraft(formData: FormData) {
  ...
  if (result.status === "blocked") {
    redirect(`/admin/leads/${leadId}?erro=Contato+bloqueado+por+opt-out`);
  }
  if (result.status === "not_found") {
    redirect(`/admin/leads/${leadId}?erro=Não+há+mensagem+recebida+para+responder`);
  }
  if (result.status === "already_replied") {
    redirect(`/admin/leads/${leadId}?erro=A+última+mensagem+já+foi+respondida`);
  }
  redirect(`/admin/leads/${leadId}?rascunho=1`);
}
...
export async function approveInstagramReply(formData: FormData) {
  ...
  if (result.status !== "sent") {
    redirect(`/admin/leads/${leadId}?erro=${errors[result.status] || "Falha+no+envio"}`);
  }
  redirect(`/admin/leads/${leadId}?enviado=1`);
}
...
export async function escalateInstagramConversation(formData: FormData) {
  ...
  revalidatePath(`/admin/leads/${leadId}`);
  revalidatePath("/admin/excecoes");
  redirect(`/admin/leads/${leadId}?escalado=1`);
}
...
export async function runInstagramReliabilityCheck() {
  ...
  if (result.status === "error") {
    redirect(`/admin/configuracoes?erro=${encodeURIComponent(result.error)}`);
  }
  if (result.status === "locked") {
    redirect("/admin/configuracoes?erro=Uma+verificação+já+está+em+andamento");
  }
  redirect(`/admin/configuracoes?verificado=${result.status}`);
}
