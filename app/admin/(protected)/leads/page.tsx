...
export default async function LeadsPage({ searchParams }: LeadsPageProps) {
  ...
  return <>
    <PageHeader eyebrow="Relacionamento" title="Leads" description="Priorize intenção real, acompanhe a origem e mantenha cada conversa no contexto certo." action={<Link href="/admin/conversas" className="inline-flex items-center gap-2 rounded-xl bg-[#173244] px-4 py-3 text-xs font-bold text-white"><UserPlus size={15}/> Entrada manual</Link>} />
      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[980px] text-left">
          ...
          <tbody className="divide-y divide-[#efefea]">{filtered.map((lead,index)=><tr className="group transition hover:bg-[#faf9f6]" key={lead.id}>
            <td className="px-5 py-4"><Link href={`/admin/leads/${lead.id}`} className="flex items-center gap-3"><Avatar name={lead.name} username={lead.instagramUsername} index={index}/><span><span className="block text-xs font-bold text-[#294653] group-hover:text-[#d96245]">{lead.name || `@${lead.instagramUsername}`}</span><span className="mt-0.5 block text-[10px] text-[#8a959b]">@{lead.instagramUsername}</span></span></Link></td>
