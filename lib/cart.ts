...
const itemSchema = z.object({
  ...
  personalization: z.string().trim().min(1).max(200).nullable().optional(),
});
...
export function parseCart(
  ...
  for (const input of parsed.data) {
    ...
    if (
      ...
      Math.min(
        MAX_ITEM_QUANTITY,
        catalog?.[item.productId]?.maximumQuantity ?? MAX_ITEM_QUANTITY,
      )
    )
      return null;
    items.set(key, item);
...
export function addCartItem(
  ...
  const incoming = parseCart([item], catalog)?.[0];
  if (!incoming)
...
export function checkoutItems(
  ...
  if (payload.items !== undefined) return parseCart(payload.items, catalog);
  return parseCart(
    [
      {
